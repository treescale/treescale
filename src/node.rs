...
impl Node {
    ...
    pub fn on(&mut self, name: &str, callback: Box<Fn(Arc<Event>)>) {
        self.event_handler.on(name, callback)
    }

    pub fn off(&mut self, name: &str) {
        self.event_handler.remove(name)
    }

    // functions with specific events
    pub fn on_pending_conn(&mut self, callback: Box<Fn(Arc<Event>)>) {
        self.event_handler.on(EVENT_ON_PENDING_CONNECTION, callback)
    }

    pub fn on_conn_close(&mut self, callback: Box<Fn(Arc<Event>)>) {
        self.event_handler.on(EVENT_ON_CONNECTION_CLOSE, callback)
    }
