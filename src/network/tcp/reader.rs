                ...
                for i in 0..command.socket_token.len() {
                    match self.reader_conns.get_mut(&command.socket_token[i]) {
                        Some(conn) => {
                            ...
                            match self.poll.reregister(&conn.socket_r, conn.socket_token, Ready::writable(), PollOpt::edge()) {
                                Ok(_) => {},
                                Err(e) => {
    ...
    fn readable(&mut self, token: Token) {
        ...
        let mut final_data: Vec<Vec<u8>> = vec![];
        {
            ...
            loop {
                ...
                match conn.socket_r.read(&mut self.readable_buffer) {
                    Ok(rsize) => {
                        // we got EOF or not
                        if rsize == 0 {
                            close_conn = true;
...

        {
            ...
            match conn.flush_write_queue() {
                Ok(end_of_q) => {
                    ...
                    match self.poll.reregister(&conn.socket_r, conn.socket_token, ready_state, PollOpt::edge()) {
                        Ok(_) => {},
                        Err(e) => {
